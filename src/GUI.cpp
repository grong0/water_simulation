...
bool GUI::GetUserInput()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        if (IsMouseButtonDown(MOUSE_BUTTON_LEFT))
        {
            ...
            if (CheckCollisionPointRec(mouseWorldPos, this->playButton))
            {
    ...
    BeginDrawing();
        ...
        // BeginMode2D(camera);
            ...
            for (size_t i = 0; i < this->holes->size(); i++)
            {
                ...
                {
                    DrawRectangle(hole.pos_x-5, windowHeight-hole.pos_y, hole.width, 10, RED);
                }
                /* code */
            }
        ...
        if (this->isUserInput)
        {
            DrawTriangle( (Vector2) {700, 75}, (Vector2){650, 50}, (Vector2) {650, 100}, GREEN);                                // Draw a color-filled triangle (vertex in counter-clockwise order!)
        }
